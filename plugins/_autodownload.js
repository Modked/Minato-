...
export async function before(m) {
    ...
    try {
        if (m.text.trim().match(fbRegex)) {
            ...
            if (hasil.length === 0) {
                await conn.sendFile(m.sender, download[0].link, '', `اسم المستخدم: \`${username}\`\n👀: \`${stats.views}\`\n♥️: \`${stats.likes}\`\n💬: \`${stats.comments}\`\n↪️:  \`${stats.shares}\`\n📥: \`${stats.downloads}\``, m);
            } else {
                for (let i of hasil) {
                    await conn.sendFile(m.sender, i.link, '', `اسم المستخدم: \`${username}\`\n👀: \`${stats.views}\`\n♥️: \`${stats.likes}\`\n💬: \`${stats.comments}\`\n↪️:  \`${stats.shares}\`\n📥: \`${stats.downloads}\``, m);
                }
            }
        }
    } catch (e) {
    ...
    try {
        let { data } = await axios.post("https://getmyfb.com/process", {
            ...
            headers: {
                ...
                "Hx-Target": "target",
                "Hx-Trigger": "form",
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, مثل Gecko) Chrome/115.0.0.0 Safari/537.36 Edg/115.0.1901.188"
            }
        });
