...
export async function before(m, { conn }) {
    ...
    if (chat.viewonce) {
        if (m.mtype == 'viewOnceMessageV2') {
            ...
            if (/video/.test(type)) {
                return conn.sendFile(m.chat, buffer, '', `[تم اكتشاف فيديو عرض مرة واحدة]\n\n` + (msg[type].caption || ''), m);
            } else if (/image/.test(type)) {
                return conn.sendFile(m.chat, buffer, '', `[تم اكتشاف صورة عرض مرة واحدة]\n\n` + (msg[type].caption || ''), m);
            }
            ...
            if (/audio/.test(tipe)) {
            await conn.reply(m.chat, `@${m.sender.split("@")[0]} قد أرسل ملف صوتي يتم تشغيله مرة واحدة`, null, {contextInfo: {mentionedJid: [m.sender]}})
