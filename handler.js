...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 7) {
                    this.reply(m.chat, `Limit kamu sidah mencapai batas\n*segera beli limit atau premium ke owner*`, m)
                ...
                try {
                    ...
                    if (!isPrems) {
                        ...
                        if (plugin.limit) return m.reply(`🔖 Limit telah digunakan: ${m.limit * 1}\nTersisa: ${global.db.data.users[m.sender].limit - 1}`)
                        }
                } catch (e) {
                    // Error occured
                    m.error = e
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (chat.sWelcome || this.welcome || conn.welcome || 'safrot4, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', await this.getName(id)).replace('@member', member).replace('@desc', groupMetadata.desc?.toString() || 'unknow')
    ...
    this.sendMessage(id, {
        ...
        mentionedJid: [user],
            externalAdReply: {
                ...
                mediaType: 1,
                mediaUrl: 'https://qu.ax/OkbeY.jpg',
                renderLargerThumbnail: true,
...
const canLea = await new canvafy.WelcomeLeave()
    .setAvatar(pp)
    .setBackground("image", "https://i.ibb.co/1G7yf9P/fa1e7dab57ee75e14ce67a61ec2a1381.jpg")
    .setTitle("Goodbye")
...
if (templ.gcImg) {
    ...
    this.sendMessage(id, {
        ...
        mentionedJid: [user],
            externalAdReply: {
                title: 'الى القاء عزيزي لاتفكر برجوع⚡',
                body: '',
                thumbnail: xnxx,
                sourceUrl: sgc,
                mediaType: 1,
...
let msg = {
        premium: '> *\`『 الميزه دي للمميزين بس يحب 🚫 』\`*',
        group: '> *\`『 الميزه دي شغاله فلرومات بس يحب 🚫 』\`*',       
        private: '> *\`『 الميزه دي شغاله في البرايفت بس 🚫 』\`*',       
        botAdmin: '> *\`『 علشان تعرف تستخدم الميزه دي ارفع البوت ادمن 🚫 』\`*',
        admin: '> *\`『 الميزه دي للادمن بس يحب 🚫 』\`*',
        restrict: '> *\`『 تقييد المميزات مش مفعّل للدردشة دي. 🚫 』\`*',
        adminonly: '> *\`『 تم تعطيل الميزة للأعضاء. 🚫 』\`*',
        premiumonly: `📢 لا يمكنك الوصول إلى الميزة ❗
اشترِ البريميوم للوصول عبر الكمبيوتر.
اتصل بالمالك من خلال الرابط أدناه:
wa.me/${global.info.nomerown}`,
        groups: '> *\`『 الميزه دي شغاله فلرومات بس يحب 🚫 』\`*',
        gconly: `📢لا يمكنك الوصول إلى الميزة ❗
انضم إلى مجموعة ♯ЅᗩFᏒOT꙯ للحصول على الوصول.\n\n> ${global.url.sgc}`,
        game: '> *\`『 الميزة *اللعبة* غير مفعّلة لهذه الدردشة.🚫 』\`*',
        rpg: '> *\`『 الميزة *آر بي جي* غير مفعّلة لهذه الدردشة.🚫 』\`*',
        rowner: '> *\`『 الميزه دي للمطور بس يحب⚡ 』\`*',
        owner: '> *\`『 الميزه دي للمطور بس يحب 🚫 』\`*',
        mods: '> *\`『 الميزه دي للمشرفين بس يحب 🚫 』\`*'
        }[type]
  ...
  if (msg) return conn.sendMessage(m.chat, {
    ...
    let daftar = {
  unreg: `\`Anda belum terdaftar di Database. Segera lakukan registrasi dengan mengetik:\`
  
- /daftar nama.umur`}[type]
  
  if (daftar) return conn.sendUrlImageButton(m.chat, daftar, [{name: "quick_reply", buttonParamsJson: `{"display_text": "Verify", "id": "@verify"}`}], wm, registrasi, fkon)
        }
...
function ucapan() {
  ...
  if (time >= 10) {
    res = "بوت سفࢪوت الكبي⚡ ☀️"
  }
  if (time >= 15) {
    res = "بوت سفࢪوت الكبي⚡ 🌇"
  }
  if (time >= 18) {
    res = "بوت سفࢪوت الكبيࢪ⚡ 🌙"
  }
  return res
}
